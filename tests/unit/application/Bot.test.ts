...
describe("Bot", () => {
  ...
  test("does not execute a tick when maxTicks is non-positive", async () => {
    const calls: string[] = [];
    const bot = new Bot(
      {
        guardRisk: {
  ...
  test("waits for async runtime disposable startup before the first quote tick", async () => {
    const calls: string[] = [];
    ...
    expect(calls).toContain("runtimeStart");
