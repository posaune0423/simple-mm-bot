...
describe("BulkMarketFeed", () => {
  test("seeds snapshot from ticker and L2 book with ns timestamps normalized to ms", async () => {
    ...
    expect(snapshot.tickerUpdatedAt).toBeGreaterThan(1_000_000_000_000);
  ...
  test("polls account and position state without refreshing market timestamps", async () => {
    ...
    expect(initial.timestamp).toBeGreaterThan(1_000_000_000_000);
  ...
  test("does not carry candle OHLCV fields into ticker and book snapshots", async () => {
    ...
    expect(snapshots.at(-2)?.open).toBeUndefined();
    expect(snapshots.at(-2)?.volume).toBeUndefined();
    expect(snapshots.at(-1)?.open).toBeUndefined();
    expect(snapshots.at(-1)?.volume).toBeUndefined();
  });
  ...
  test("does not poll REST just because seeded exchange timestamps are stale", async () => {
    const logs = captureLogs();
    let tickerCalls = 0;
    let bookCalls = 0;
    ...
    try {
      const feed = new BulkMarketFeed(client, {
        market: "BTC-USD",
        marketWsReconnectAfterMs: 10_000,
      });
      ...
      expect(snapshot.bestBid).toBe(99);
      expect(snapshot.bestAsk).toBe(101);
      expect(snapshot.markPrice).toBe(100);
  ...
  test("reconnects websocket subscriptions when the book stream is stale and ticker stays fresh", async () => {
    let subscribeCalls = 0;
    let unsubscribeCalls = 0;
    let closeCalls = 0;
    const staleTs = Date.now() - 10_000;
    ...
    const feed = new BulkMarketFeed(client, {
      market: "BTC-USD",
      marketWsReconnectAfterMs: 5_000,
    });

    await feed.connect();
    const now = Date.now();
    Object.assign(feed as unknown as Record<string, unknown>, {
      wsSubscribedAtMs: now - 10_000,
      lastWsBookReceivedAtMs: now - 10_000,
      lastWsTickerReceivedAtMs: now,
    });
    await waitFor(() => subscribeCalls >= 6);
    await feed.disconnect();
  ...
  test("keeps websocket subscriptions when received frames carry stale exchange timestamps", async () => {
    ...
    const client = {
      market: {
        async ticker() {
          return { markPrice: 100, timestamp: Date.now() * 1_000_000 };
        },
        async l2Book() {
          return {
            levels: [[{ px: 99, sz: 1 }], [{ px: 101, sz: 1 }]],
  ...
  test("does not resubscribe after disconnect during in-flight stale recovery", async () => {
    ...
    const disconnect = feed.disconnect();
    await waitFor(() => closeResolvers.length >= closeCalls);
