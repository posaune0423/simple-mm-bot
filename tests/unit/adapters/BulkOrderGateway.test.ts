...
describe("BulkOrderGateway", () => {
  ...
  test("does not re-emit fills that fall outside the seen-fill TTL window", async () => {
    const oldTimestamp = 1_700_000_010_000;
    const newTimestamp = oldTimestamp + 2_000;
    const pollFills = [
      { orderIdMaker: "new-maker", orderIdTaker: "new-taker", timestamp: newTimestamp },
      { orderIdMaker: "old-maker", orderIdTaker: "old-taker", timestamp: oldTimestamp },
      { orderIdMaker: "old-maker", orderIdTaker: "old-taker", timestamp: oldTimestamp },
    ];
    let poll = 0;
    const gateway = new BulkOrderGateway(
            ...
            return [
              {
                ...
                taker: "other",
                orderIdMaker: fill.orderIdMaker,
                orderIdTaker: fill.orderIdTaker,
                isBuy: true,
                symbol: "BTC-USD",
                amount: 0.1,
                price: 100,
    ...
    const fills: Array<{ quoteId: string }> = [];
    gateway.subscribeFills((fill) => {
      if (fill.quoteId === undefined) {
        throw new Error("expected fill quote id");
      }
      fills.push({ quoteId: fill.quoteId });
    });
    ...
    expect(fills).toEqual([{ quoteId: "new-maker" }]);
  });
