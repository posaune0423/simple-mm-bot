...
describe("resolveBacktestPaperLoopOptions", () => {
  test("defaults to the shared SQLite DB instead of an artifact-local loop DB", () => {
    ...
    try {
      ...
      expect(options.outputDir).toBe(join("artifacts/strategy-runs", "20260219-000000-loop"));
  ...
  test("allows explicit DB_PATH and --db override while keeping artifacts separate", () => {
    const previousDbPath = Bun.env.DB_PATH;
    ...
    try {
      ...
      expect(fromEnv.outputDir).toBe(join("artifacts/strategy-runs", "20260219-000000-sweep-a"));
