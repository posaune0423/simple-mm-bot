...
export function serializeRun(run: TelemetryRun): typeof telemetryRunsTable.$inferInsert {
  return {
    id: run.id,
    mode: run.mode,
    venue: run.venue,
...
export function deserializeRun(row: TelemetryRunRow): TelemetryRun {
  return {
    id: row.id,
...
export function serializeEvent(event: TelemetryEvent): typeof telemetryEventsTable.$inferInsert {
  return {
    id: event.id,
    runId: event.runId,
    mode: event.mode,
...
export function deserializeEvent(row: TelemetryEventRow): TelemetryEvent {
  return {
    id: row.id,
    runId: row.runId,
    mode: row.mode as AppMode,
