...
flowchart LR
    ...
    subgraph Application["Application"]
        ...
        QuotingCycle["QuotingCycleService\nstrategy + intents + reconcile"]
    ...
    Bot --> UseCases
    Bot --> MetricsRec
    UseCases --> QuotingCycle
    QuotingCycle --> Strategy
    QuotingCycle --> IntentBuilder
    QuotingCycle --> Reconciler
    UseCases --> Ports
    UseCases --> Reconciler
    MetricsRec --> Ports
...
| `OrderReconciler`             | 前回 order と今回 intent の差分 reconcile、必要な cancel / replace / reuse                                           | quote 価格計算、risk 判定            |
...
現在の `Bot.runTick()` は、risk check、event drain、inventory reduction、quoting cycle、historical feed advance の順で動く。
...
sequenceDiagram
    ...
    participant Cycle as QuotingCycleService
    participant Strategy as Strategy
    participant Builder as OrderIntentBuilder
    participant Reconciler as OrderReconciler
    ...
    alt EMERGENCY_STOP
        ...
        alt riskState == OK and didReduceInventory == false
            Bot->>Cycle: execute()
            Cycle->>Feed: getSnapshot()
            Cycle->>Pos: get()
            opt qualityGate enabled
                Cycle->>Feedback: getRecentSideMarkoutFeedback()
            end
            Cycle->>Strategy: decide(snapshot, position, quality)
            Strategy->>QE: compute(QuoteEngineInput)
            QE-->>Strategy: Quote
            Strategy-->>Cycle: StrategyDecision.quote
            Cycle->>Metrics: recordQuote()
            loop each side / level
                Cycle->>Feed: getSnapshot()
                Cycle->>Metrics: recordMarketSnapshot()
                Cycle->>Cycle: stale touch / momentum guard / guarded limit price
            end
            Cycle->>Builder: build(Quote, placement context)
            Builder-->>Cycle: OrderIntent[]
            Cycle->>Reconciler: reconcile(intents)
            Reconciler->>GW: cancel stale/replaced orders
            Reconciler->>GW: place missing/replaced orders
            Reconciler-->>Cycle: ReconcileResult
        else paused or reducing
            Bot-->>Bot: skip quoting cycle
        end
...
- `ReduceInventoryUseCase` が注文した tick では `QuotingCycleService` をスキップする。
- `QuotingCycleService` は通常 tick で blanket `cancelAll()` しない。`OrderReconciler` が差分 cancel / replace を行う。
- `OrderReconciler` が cancel 失敗で注文状態を信頼できない場合だけ、unknown state として `cancelAll()` に倒す。
...
- `OK`: quoting cycle 可能。
- `PAUSE_QUOTING`: quoting cycle はスキップ。event drain と inventory reduction は継続。
...
- [ ] `Bot` の tick 順序を変える時は、risk state、event drain、inventory reduction、quoting cycle、cleanup の相互作用を更新する。
- [ ] 通常 tick の quote 更新で blanket `cancelAll()` を増やさない。必要な場合は `OrderReconciler` の unknown-state fallback として扱う。
